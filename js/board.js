...
function handleFilterTasks() {
  ...
  if (searchTerm.length == 0) {
    ...
    filteredTasks = currentUserTasks.filter((filteredTasks) => {
      return (
        filteredTasks.title.toLowerCase().includes(searchTerm) ||
        filteredTasks.description.toLowerCase().includes(searchTerm)
      );
    });
    filterTasksByStatus();
  }
...
function filterTasksByStatus() {
  ...
  let statusInProgress = filteredTasks.filter(
    (task) => task.status == 'inProgress'
  );
  let statusAwaitingFeedback = filteredTasks.filter(
...
function renderProgressBar(element) {
  ...
  if (subTasksInTask.length > 0) {
    let taskProgressContainer = document.getElementById(
      'task_progress_' + taskId
    );
    ...
    fill.style.width = `${
      (subTasksInTasksDone.length / subTasksInTask.length) * 100
    }%`;
...
function showDetailWindow(id) {
  ...
  for (
    let filteredTasksIndex = 0;
    filteredTasksIndex < filteredTasks.length;
    filteredTasksIndex++
  ) {
...
function renderAssignedContactsDetails() {
  let detailAssignedContacts = document.getElementById(
    'detail_assigned_contacts'
  );
  for (
    let assignedContactsIndex = 0;
    assignedContactsIndex < currentTask.user.length;
    assignedContactsIndex++
  ) {
...
function renderAssignedSubTasks(id) {
  ...
  for (
    let assignedSubTaskIndex = 0;
    assignedSubTaskIndex < currentTask.subTasks.length;
    assignedSubTaskIndex++
  ) {
    const currentSubTask = currentTask.subTasks[assignedSubTaskIndex];
    detailAssignedSubTasks.innerHTML += renderAssignedSubTasksTemplate(
      currentSubTask,
      assignedSubTaskIndex,
      id
    );
...
function isSubTaskDone(currentSubTask, assignedSubTaskIndex) {
  let subTaskCheckbox = document.getElementById(
    `subTask_${assignedSubTaskIndex}`
  );
  let subTaskTitle = document.getElementById(
    `subTask_title_${assignedSubTaskIndex}`
  );
...
function subTaskDone(currentSubTasks) {
  for (
    ...
    currentSubTaskIndex < currentSubTasks.length;
    currentSubTaskIndex++
  ) {
    const currentSubTask = currentSubTasks[currentSubTaskIndex];
    let subTaskCheckbox = document.getElementById(
      `subTask_${currentSubTaskIndex}`
    );
    let subTaskTitel = document.getElementById(
      `subTask_title_${currentSubTaskIndex}`
    );
...
function showSelectedPriorityUrgent() {
  document
    .getElementById('editPriorityUrgent')
    .classList.add('prio-urgent-selected');
  document
    .getElementById('editPriorityMedium')
    .classList.remove('prio-medium-selected');
  document
    .getElementById('editPriorityLow')
    .classList.remove('prio-low-selected');

  document.getElementById('editPriorityUrgentImg').src =
    'assets/img/prio-urgent-white.png';
  document.getElementById('editPriorityMediumImg').src =
...
function showSelectedPriorityMedium() {
  document
    .getElementById('editPriorityMedium')
    .classList.add('prio-medium-selected');
  document
    .getElementById('editPriorityUrgent')
    .classList.remove('prio-urgent-selected');
  document
    .getElementById('editPriorityLow')
    .classList.remove('prio-low-selected');
...
function showSelectedPriorityLow() {
  ...
  document.getElementById('editPriorityMediumImg').src =
    'assets/img/prio-medium.png';
  document.getElementById('editPriorityLowImg').src =
...
function editShowSubTasks(id) {
  ...
  for (
    ...
    detailAssignedSubTasks.innerHTML += editShowSubTasksTemplate(
      currentSubTask,
      assignedSubTaskIndex,
      id
    );
...
async function saveChangedTask(currentTaskId) {
  ...
  let changedDescription = document
    .getElementById('changed_description')
    .value.replace(/\n/g, '<br>\n');
...
async function deleteTask(currentTaskId) {
  let taskToDelete = filteredTasks.findIndex(
    (taskId) => taskId.id == currentTaskId
  );
