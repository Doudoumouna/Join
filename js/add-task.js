...
function renderSubTask() {
  ...
  for (let subTaskIndex = 0; subTaskIndex < subTasks.length; subTaskIndex++) {
    ...
    document.getElementById('addSubtaskCheckbox').innerHTML +=
      showSubtaskCheckbox(subTask, subTaskIndex);
  }
}
...
function showUsers(contact) {
  ...
  if (selectorcontactIndex == 0) {
    ...
    for (let i = 0; i < activUserContact.length; i++) {
      document.getElementById('selector_user_dropdown').innerHTML +=
        showContactsDropDown(i, activUserContact, currentUser);
    }
    if (!(contact == 0)) {
      document.getElementById('selector_user_dropdown').innerHTML +=
        showInviteNewContact();
    }
    if (contact == 0) {
...
function selectedUserAdd(contactInitials, contactcolor, contactname) {
  document
    .getElementById(
      'user_select' + contactInitials + contactcolor + contactname
    )
    .classList.add('checked');
  document.getElementById(
    'user_select' + contactInitials + contactcolor + contactname
  ).src = 'assets/img/userSelect-selected.png';
}
...
function selectedUser(contactInitials, contactcolor, contactname) {
  ...
  if (
    ...
    document.getElementById(
      'user_select' + contactInitials + contactcolor + contactname
    ).src = 'assets/img/userSelect-img.png';
  } else if (look < 0) {
  } else {
    userSelect.push({
...
function findeContactIndex(contactcolor) {
  let index;
  for (let i = 0; i < userSelect.length; i++) {
    if (userSelect[i].concolor == contactcolor) index = i;
  }
  return index;
}
...
function showTaskCategories() {
  if (selectorCategoryIndex == 0) {
    ...
    for (let n = 0; n < currentUser.category.length; n++) {
      ...
      document.getElementById('selector_Category_Dropdown').innerHTML +=
        showExistingCategories(staticCategorys);
    }
...
function selectedCategory(category, color) {
  ...
  document.getElementById('category_selector').innerHTML = showSelectCategory(
    category,
    color
  );
  document.getElementById('selector_Category_Dropdown').innerHTML = '';
...
function addCategoryColor(value) {
  if (document.getElementById('input-new-category').value) {
    ...
    document.getElementById(
      'categoryColorCells'
    ).innerHTML = /*html*/ ` <img class="chosen-color" src="./assets/img/${categorySelectedColor}.png" alt="">`;
    document.getElementById('alert_message').innerHTML = '';
  } else {
    document.getElementById(
      'alert_message'
    ).innerHTML = `Please enter category first!`;
  }
}
...
async function addCategory() {
  ...
  if (categorySelectedColor && newCategory) {
    ...
    document.getElementById(
      'alert_message'
    ).innerHTML = `Please select a category name and then a color!`;
  }
}
...
function urgentPrioritySelected() {
  ...
  document.getElementById('priorityUrgentImg').src =
    'assets/img/prio-urgent-white.png';
  document.getElementById('priorityMediumImg').src =
...
function lowPrioritySelected() {
  ...
  document.getElementById('priorityMediumImg').src =
    'assets/img/prio-medium.png';
  document.getElementById('priorityLowImg').src =
