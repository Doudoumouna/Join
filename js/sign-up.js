...
function checkIsUserRegistered(userName, userEmail, userPassword) {
  let indexOfName = users.findIndex(
    (user) => user.nameMatchCode == userName.toLowerCase()
  );
  let indexOfEmail = users.findIndex(
    (user) => user.emailMatchCode == userEmail.toLowerCase()
  );
